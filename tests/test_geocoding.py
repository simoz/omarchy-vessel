...
def feature(name="Genova", country="Italy", countrycode="IT", point=None):
    return dict(
        properties=dict(
            name=name, country=country, countrycode=countrycode, state="Liguria"
        ),
...
class CityTest(unittest.TestCase):
    def test_ambiguous_cities_remain_separate_and_invalid_positions_are_ignored(self):
        result = geocoding.places(
            dict(
                features=[
                    feature(),
                    feature(),
                    feature("Genova", "Philippines", "PH", [124.19, 9.65]),
                    feature(point=[190, 95]),
                    None,
                    dict(properties={}),
                ]
            )
    ...
    def test_request_encodes_city_and_never_includes_credentials(self):
        with (
            patch.dict(os.environ, {"AISSTREAM_API_KEY": "secret-test"}),
            patch.object(network, "build_opener") as opener,
        ):
            opener.return_value.open.return_value.__enter__.return_value.read.return_value = json.dumps(
                dict(features=[feature()])
            ).encode()
            self.assertEqual(
                geocoding.search("São Paulo & coast")[0]["label"], "Genoa (Italy)"
            )
            outgoing = opener.return_value.open.call_args.args[0]
            self.assertEqual(
                parse_qs(urlsplit(outgoing.full_url).query)["q"], ["São Paulo & coast"]
            )
    ...
    def test_selected_city_survives_restart_without_another_lookup(self):
        with (
            ...
            output = io.StringIO()
            with (
                patch("runtime.ensure_runtime"),
                patch.object(vessel.basemap, "build", return_value={}),
                patch.object(vessel.Receiver, "run", new_callable=AsyncMock),
                patch.object(geocoding, "fetch_json") as fetch,
            ...
            with (
                patch("runtime.ensure_runtime"),
                patch.object(vessel.basemap, "build", return_value={}),
                patch.object(vessel, "Receiver") as receiver,
                patch.object(vessel.asyncio, "run"),
                redirect_stdout(io.StringIO()),
            ):
                vessel.main(["--saved-settings"])
                self.assertEqual(
                    receiver.call_args.args[1]["location"], "Genoa (Italy)"
                )
