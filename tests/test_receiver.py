...
from vessel import Fleet, Receiver
...
class ReceiverTest(unittest.IsolatedAsyncioTestCase):
    async def test_binary_compression_subscription_and_clean_cancellation(self):
        ...
        async def peer(socket):
            observed["compression"] = socket.response.headers.get("Sec-WebSocket-Extensions", "")
            observed["subscription"] = json.loads(await socket.recv())
            await socket.send(json.dumps(dict(MessageType="SubscriptionConfirmation")).encode())
            await socket.send(json.dumps(dict(MessageType="PositionReport",MetaData=dict(MMSI=123456789,ShipName="LOCAL TEST"),
                Message=dict(PositionReport=dict(Valid=True,Latitude=0,Longitude=0.1,Sog=4,Cog=90)))).encode())
    ...
    async def test_rejection_is_terminal_and_redacts_server_message(self):
        ...
        async with serve(peer,"127.0.0.1",0) as server:
            receiver = Receiver(Fleet(0,0,25),{},"secret-key",snapshots.append,
                url=f"ws://127.0.0.1:{server.sockets[0].getsockname()[1]}")
    ...
    async def test_reconnect_after_closed_transport(self):
        ...
        async def peer(socket):
            ...
            try:
                ...
                if connections == 1:
                    ...
                    await socket.send(json.dumps(dict(MessageType="SubscriptionConfirmation")))
                    ready.set()
                    await socket.wait_closed()
            finally:
                active -= 1
        snapshots = []
        async with serve(peer,"127.0.0.1",0) as server:
            receiver = Receiver(Fleet(0,0,25),{},"test",snapshots.append,retry_delay=0.02,
                url=f"ws://127.0.0.1:{server.sockets[0].getsockname()[1]}")
            task = asyncio.create_task(receiver.run())
            try:
                ...
                self.assertTrue(any(s.get("status") == "RECONNECTING" for s in snapshots))
            finally:
                task.cancel()
                with suppress(asyncio.CancelledError):
                    await task

    @unittest.skipUnless(shutil.which("openssl"), "openssl required for temporary TLS certificate")
    async def test_untrusted_tls_certificate_cannot_receive_subscription(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            async with serve(peer,"127.0.0.1",0,ssl=context) as server:
                receiver = Receiver(Fleet(0,0,25),{},"never-send-this",output,
