...
def report(kind="PositionReport", mmsi=123456789, **body):
    return dict(
        MessageType=kind,
        MetaData=dict(MMSI=mmsi),
        Message={
            kind: dict(Valid=True, Latitude=0, Longitude=0.1, Sog=4, Cog=90) | body
        },
    )
    ...
    def test_circle_fits_subscription_even_near_dateline_and_poles(self):
        for lat, lon in [
            ...
            for bearing in range(0, 360, 5):
                ...
                self.assertTrue(
                    any(a[0] <= y <= b[0] and a[1] <= x <= b[1] for a, b in boxes)
                )
...
class FleetTest(unittest.TestCase):
    def test_class_a_and_b_and_static_merge(self):
        ...
        fleet.ingest(
            report(
                "StaticDataReport",
                123456780,
                ReportA=dict(Valid=True, Name=" TEST@@ "),
                ReportB=dict(Valid=True, ShipType=36),
            ),
            1100,
        )
    ...
    def test_filter_limits_and_malformed_input(self):
        ...
        for bad in [
            ...
            [],
            {},
            dict(MessageType="PositionReport", MetaData=[], Message={}),
            report(mmsi=42),
        ]:
        ...
        fleet = ais.Fleet(0, 0, 1)
        fleet.ingest(report(), 1000)
        self.assertEqual(fleet.snapshot(1000)["total"], 0)

    def test_timestamp_provenance(self):
        self.assertEqual(ais.message_time({}, 1000), (1000, "Received"))
        self.assertEqual(
            ais.message_time({"time_utc": "2099-01-01T00:00:00Z"}, 1000),
            (1000, "Received"),
        )
        self.assertEqual(
...
class SettingsTest(unittest.TestCase):
    ...
    def test_key_over_stdin_is_never_echoed(self):
        result = subprocess.run(
            [sys.executable, "-B", str(ROOT / "backend/vessel.py"), "--save-settings"],
...
class BasemapTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.genova = basemap.build(*g.GENOA, 25)
    ...
    def land(cls, point, data):
        return any(
            cls.inside(point, rings[0])
            and not any(cls.inside(point, r) for r in rings[1:])
    ...
    def test_offline_coastal_labels_are_geographically_projected(self):
        ...
        self.assertTrue(
            all(math.hypot(city["x"], city["y"]) <= 1.000001 for city in cities)
        )
        self.assertEqual(
    ...
    def test_clipping_and_missing_map(self):
        self.assertEqual(
            basemap.clip_segment([-1, 0], [3, 2], [0, 0, 1, 1]), [[0, 0.5], [1, 1]]
        )
