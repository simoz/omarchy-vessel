...
def clip_ring(ring, box):
    ...
    for axis, edge, direction in ((0,box[0],1),(0,box[2],-1),(1,box[1],1),(1,box[3],-1)):
        if not points:
            break
...
def densify(points):
    ...
    for a, b in zip(points, points[1:]+points[:1]):
        steps = max(math.ceil(abs(b[0]-a[0])/0.25), math.ceil(abs(b[1]-a[1])/0.25), 1)
        result.extend([[a[0]+(b[0]-a[0])*n/steps, a[1]+(b[1]-a[1])*n/steps] for n in range(steps)])
    return result
...
def build(lat, lon, radius, path=PATH):
    ...
    try:
        ...
        if dataset["version"] != 1:
            return result
        boxes = [[a[1], a[0], b[1], b[0]] for a,b in bounding_boxes(lat, lon, radius*1.5)]
        def project(point):
            distance, bearing = distance_bearing(lat, lon, point[1], point[0])
            angle = math.radians(bearing)
            return [round(math.sin(angle)*distance/radius,5), round(-math.cos(angle)*distance/radius,5)]
        polygons, coastlines = [], []
        for box in boxes:
            ...
            for line in dataset["coast"]:
                if overlaps(line["bbox"], box):
                    coastlines.extend([[project(point) for point in part] for part in clip_line(line["points"],box)])
