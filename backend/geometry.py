...
def bounding_boxes(lat, lon, radius):
    ...
    south, north = max(lat-math.degrees(angle), -90), min(lat+math.degrees(angle), 90)
    if south == -90 or north == 90:
        return [[[south, -180], [north, 180]]]
    delta = math.degrees(math.asin(min(math.sin(angle)/math.cos(math.radians(lat)), 1)))
...
def destination(lat, lon, distance, bearing):
    ...
    other_phi = math.asin(math.sin(phi)*math.cos(angle) + math.cos(phi)*math.sin(angle)*math.cos(direction))
    other_lam = lam + math.atan2(math.sin(direction)*math.sin(angle)*math.cos(phi),
