...
def run_command(args, timeout):
    ...
    child = subprocess.Popen(args, env=env, stdout=subprocess.DEVNULL,
...
def ensure_runtime(output):
    ...
    with (target.parent / (target.name + ".lock")).open("w") as lock:
        ...
        if python.exists():
            try:
                run_command([str(python), "-I", "-c", "import websockets; assert websockets.__version__ == '17.1'"], 10)
                ready = True
        ...
        if not ready:
            ...
            run_command([str(python), "-I", "-m", "pip", "--isolated", "install",
                         "--disable-pip-version-check", "--no-input", "--no-deps",
                         "--only-binary=:all:", "--require-hashes", "--retries", "2",
                         "--timeout", "20", "-r", str(REQUIREMENTS)], 180)
    env = dict(os.environ)
    env.pop("PYTHONPATH", None)
    env.pop("PYTHONHOME", None)
    # Replace this process, retaining the same PID for Quickshell lifecycle control.
    os.execve(str(python), [str(python), "-B", str(ROOT/"backend"/"vessel.py"), *sys.argv[1:]], env)
