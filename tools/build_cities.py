...
def build(source, coastline):
    ...
    for line in coastline["coast"]:
        for a,b in zip(line["points"],line["points"][1:]):
            ...
            for x in range(math.floor(min(a[0],east)),math.floor(max(a[0],east))+1):
        ...
        for line in io.TextIOWrapper(raw,encoding="utf-8"):
            ...
            for x in range(math.floor(lon-dx),math.floor(lon+dx)+1):
                ...
                for y in range(math.floor(lat-dy),math.floor(lat+dy)+1):
                    if any(segment_distance(lon,lat,a,b)<=COAST_NM for a,b in grid.get(((x+180)%360-180,y),())):
                        near=True;break
            if near:
                name="Genoa" if row[0]=="3176219" else row[1]
                cities.append(dict(id=int(row[0]),name=name,latitude=lat,longitude=lon,population=int(row[14]),country=row[8]))
    cities.sort(key=lambda city:(-city["population"],city["id"]))
    return dict(version=1,source="GeoNames cities5000",sourceSha256=hashlib.sha256(Path(source).read_bytes()).hexdigest(),coastNm=COAST_NM,cities=cities)
...
if __name__ == "__main__":
    ...
    print(f"Wrote {len(result['cities'])} coastal settlements ({output.stat().st_size} bytes)")
